...
const getImagePath = (id: number) => {
  const basePath = '/img/Sweden/Sweden';
...
const PAGE_CONTENT = {
  title: 'Sweden',
  description: 'Stockholm\'s frozen canals really emphasize the city\'s unique culture and history.',
  header: {
    backgroundImage: '/img/Sweden/sweden-panorama (1).jpg',
    altText: 'Sweden Panorama'
  },
  panoramas: {
    description: 'Frozen Forests look awesome from above.',
...
const getPanoramaPath = (id: number) => {
  return `/img/Sweden/sweden-panorama (${id}).jpg`;
};

//STEP FOUR - change image number and panorama size below
export default function SwedenGallery() {
  // Generate gallery images with useMemo
  const galleryImages = useMemo<GalleryImage[]>(() => {
    return Array.from({ length: 319 }, (_, i) => {
