...
const PAGE_CONTENT = {
  title: 'Finland',
  description: 'The land of a thousand lakes, saunas, and the magical northern lights. From Helsinki design district to the unique neighborhood of Freetown Christiania, Finland was a 10/10.',
  header: {
    backgroundImage: '/img/Finland/finland-panorama (1).jpg',
    altText: 'Finland Panorama'
  },
  panoramas: {
    description: 'Finnish landscapes go hard as desktop backgrounds.',
...
// Generate redbull images with default alt text
const getRedbullPath = (id: number) => {
  return `/img/Finland/finland-redbull (${id}).jpg`;
};

// Generate panorama images with default alt text
const getPanoramaPath = (id: number) => {
  return `/img/Finland/finland-panorama (${id}).jpg`;
};

//STEP FOUR - change image number and panorama size below
export default function FinlandGallery() {
  ...
  // Generate redbull images with useMemo
  const redbullImages = useMemo<GalleryImage[]>(() => {
    return Array.from({ length: 191 }, (_, i) => {
      const image: GalleryImage = {
        id: i + 1,
        src: getRedbullPath(i + 1),
        alt: `Redbull Photo ${i + 1}`
      };
      return image;
    });
  }, []);
  ...
  const navigateImage = (direction: 'next' | 'prev') => {
    ...
    const currentImages = currentView === 'redbull' ? redbullImages : galleryImages;
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900">
      ...
      <section className="w-full bg-gray-900 py-12">
        <div className="w-full flex justify-center px-4">
          <div className="flex items-center justify-center gap-8 md:gap-16 lg:gap-32">
            ...
            <GradientButton
              variant={currentView === 'redbull' ? 'variant' : 'default'}
              className="px-6 md:px-10 py-3 md:py-5 text-sm md:text-lg font-bold transform scale-100 md:scale-125 lg:scale-150 origin-center"
              onClick={() => setCurrentView('redbull')}
            >
              Redbull x HeavyMetal
            </GradientButton>
          </div>
        </div>
      </section>
            ...
            <div className="w-full bg-gray-900 py-12">
              <div className="w-full flex justify-center">
                <div className="w-full max-w-4xl px-4">
                  <div className="w-full text-center">
                    <h2 className="text-2xl md:text-3xl font-bold text-white">
                      Redbull x HeavyMetal - Finland Edition
                    </h2>
                  </div>
                </div>
              </div>
      ...
      {selectedIndex !== null && (
        <ImageModal
          images={currentView === 'redbull' ? redbullImages : galleryImages}
...
type GalleryView = 'photos' | 'panoramas' | 'drone' | 'redbull';
