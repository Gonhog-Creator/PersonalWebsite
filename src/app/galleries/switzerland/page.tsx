...
export const galleryImages: GalleryImage[] = Array.from({ length: 50 }, (_, i) => {
  const id = i + 1;
  return {
    id,
    src: getImagePath(id),
...
const imageDetails: Record<number, { alt: string }> = {
  ...
  // Example:
  // 1: { alt: 'Matterhorn at sunrise' },
  // 2: { alt: 'Lucerne Chapel Bridge' },
};
...
export default function SwitzerlandGallery() {
  ...
  return (
    <div className="min-h-screen bg-white">
      <GalleryNavbar currentPath="/galleries/switzerland" />

      <ProjectHeader
        title="Switzerland"
        subtitle="Photography collection from Switzerland"
        backgroundImage="/img/Switzerland/hero.jpg"
      />
          ...
          <div className="space-y-10">
            {[1, 2, 3].map((id) => (
              <div key={id} className="w-full group relative" style={{ marginBottom: '40px' }}>
                <PanoramaViewer
                  src={`/img/Switzerland/panorama-switzerland-${id}.JPG`}
                  location="Switzerland"
                />
              </div>
            ))}
