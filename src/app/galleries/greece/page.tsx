...
const imageDetails: Record<number, { alt: string }> = {
  ...
  // Example:
  // 1: { alt: 'Santorini sunset with white buildings' },
  // 2: { alt: 'Acropolis of Athens at golden hour' },
};
...
export default function GreeceGallery() {
  ...
  return (
    <div className="min-h-screen bg-white">
      <GalleryNavbar currentPath="/galleries/greece" />

      <ProjectHeader
        title="Greece"
        subtitle="Photography collection from Greece"
        backgroundImage="/img/Greece/hero.jpg"
      />
          ...
          <div className="space-y-10">
            {[1, 2, 3].map((id) => (
              <div key={id} className="w-full group relative" style={{ marginBottom: '40px' }}>
                <PanoramaViewer
                  src={`/img/Greece/panorama-greece-${id}.JPG`}
                  location="Greece"
                />
              </div>
            ))}
